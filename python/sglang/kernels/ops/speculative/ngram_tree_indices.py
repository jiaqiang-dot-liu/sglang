...
Derives, from a [bs, n, n] boolean draft tree mask where ``mask[b, i, j]`` is
"j is an ancestor of i" (n = draft_token_num):

* ``positions[b, i]``             = ancestor count of node i + seq_len[b]
* ``retrieve_index[b, i]``        = b * n + i, the flat row index
* ``retrieve_next_token[b, i]``   = i's first child, else -1
* ``retrieve_next_sibling[b, i]`` = i's next sibling under the same parent, else -1

Sibling rule, matching the CUDA kernel: candidate c > i is a sibling of i iff
parent(i) is an ancestor of c and no node in (parent(i), c) is an ancestor of c.

The ROCm build of sgl_kernel does not register this op -- ngram_utils.cu is absent
from setup_rocm.py and common_extension_rocm.cc never declares it. Drop this port
once the op is built on HIP.
"""
...
def _reconstruct_indices_kernel(
    ...
    tl.store(retrieve_index_ptr + token_base + tid, (token_base + tid).to(tl.int64))
    ...
    if next_token >= n:
        next_token = -1
    tl.store(
        retrieve_next_token_ptr + token_base + tid, next_token.to(tl.int64)
    )

    # ---- next_sibling: first c > tid whose parent == parent(tid) ----------
    # Candidate c qualifies iff (a) mask[b, c, parent] == 1 and (b) no j in
    # (parent, c) is an ancestor of c, i.e. c's parent is exactly parent(tid)
    # rather than a deeper descendant. Both scans exclude c itself, matching
    # the CUDA kernel.
    if parent >= 0:
        ...
        # (b) count ancestors of candidate strictly between parent and candidate,
        # via a (BLOCK, BLOCK) gather: bit[c, j] = mask[b, c, j] if parent < j < c.
        ...
        next_sibling = -1
    tl.store(
        retrieve_next_sibling_ptr + token_base + tid, next_sibling.to(tl.int64)
    )
...
def reconstruct_indices_from_tree_mask_triton(
    ...
    retrieve_index: torch.Tensor,  # mutable, out
    retrieve_next_token: torch.Tensor,  # mutable, out
    retrieve_next_sibling: torch.Tensor,  # mutable, out
    batch_size: int,
    draft_token_num: int,
) -> None:
    ...
    in-place fills ``positions`` / ``retrieve_index`` / ``retrieve_next_token`` /
    ``retrieve_next_sibling`` from the flattened [bs, n, n] boolean tree mask.
    """
