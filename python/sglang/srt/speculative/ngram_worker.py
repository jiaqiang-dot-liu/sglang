...
if is_hip():
    ...
    # `reconstruct_indices_from_tree_mask` (ngram_utils.cu is absent from
    # setup_rocm.py and common_extension_rocm.cc never declares the op), so
    # the CUDA import raises AttributeError and every NGRAM config variant
    # dies at warmup. Fall back to the Triton port with identical semantics.
